...
# Load data bersih
df = pd.read_csv("Merge Datas (1).csv")

# Pastikan kolom tanggal dalam datetime
df['Tanggal Transaksi Terakhir'] = pd.to_datetime(df['Tanggal Transaksi Terakhir'], errors='coerce')

# Pastikan kolom Tahun dan Bulan ada (kalau belum, buat lagi)
...
# ============ Streamlit UI ============ #
st.set_page_config(page_title="Dashboard Rekap KPPN", layout="wide")
st.title("Dashboard Rekap KPPN")
...
# Tampilkan metrik
st.subheader(f"Saldo Bulan {selected_month} {selected_year}")
col1, col2, col3 = st.columns(3)
col1.metric("Telah Menjadi Hak Negara", format_rupiah(saldo_hak_negara))
col2.metric("Rekening Antara", format_rupiah(saldo_rek_antara))
col3.metric("Belum Menjadi Hak Negara", format_rupiah(saldo_belum_hak))
